...
export default function DimeCuandoApp() {
    ...
    if (status === 'error') {
      const msg =
        error?.message?.includes('429') || error?.message?.toLowerCase().includes('limit')
          ? 'Llegaste al límite del demo (10/día). Escríbeme a emmanuel@alanis.dev'
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-100 overflow-hidden">
      <header className="shrink-0 flex items-center justify-between h-14 px-4 border-b border-zinc-800/80 bg-zinc-950/95 backdrop-blur-sm z-10">
        ...
        >
          <ArrowLeft className="h-3.5 w-3.5 text-zinc-500 group-hover:text-emerald-400 transition-colors" />
        ...
        <div className="flex items-center gap-1.5 rounded-full bg-zinc-900 border border-zinc-800 px-2.5 sm:px-3 py-1 sm:py-1.5 shrink-0">
          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
          <span className="text-[11px] sm:text-xs text-zinc-400 font-medium">
            <span className="hidden sm:inline">Conectado: </span>Mock Inbox
          </span>
        </div>
      </header>

      <div className="flex flex-1 min-h-0">
        <div className="flex flex-col w-full md:w-[60%] md:border-r border-zinc-800/80 min-h-0">
          <div className="flex-1 overflow-y-auto min-h-0">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-2 px-6 text-center">
                <p className="text-sm font-medium text-zinc-300">
                  Qué onda, soy tu agente de cobros
                </p>
                <p className="text-xs text-zinc-500 max-w-xs leading-relaxed">
                  Reviso tus facturas vencidas y te redacto el mensaje justo
                  para cada cliente. Tú nada más le picas a copiar.
                </p>
              </div>
            ) : (
        ...
        {/* Right panel: inline on desktop, bottom sheet on mobile */}
        <div
          className={cn(
            'flex flex-col bg-zinc-950 min-h-0',
            // Desktop: inline 40%
            'md:flex md:w-[40%] md:relative md:inset-auto md:border-t-0',
            // Mobile: fixed bottom sheet, hidden by default
            'fixed inset-x-0 bottom-0 z-40 max-h-[75vh] border-t border-zinc-800 rounded-t-2xl md:rounded-none shadow-2xl md:shadow-none transition-transform duration-300',
            mobilePanelOpen ? 'translate-y-0' : 'translate-y-full md:translate-y-0',
          )}
        >
          {/* Mobile-only drag handle */}
          <div className="md:hidden mx-auto mt-2 mb-1 h-1 w-10 rounded-full bg-zinc-700" aria-hidden="true" />
          <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-zinc-800/80">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="md:hidden flex h-7 w-7 items-center justify-center rounded-md text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 transition-colors"
                aria-label="Cerrar"
              >
                <X className="h-4 w-4" />
              </button>
        ...
        {/* Mobile-only backdrop when sheet is open */}
        {mobilePanelOpen && (
          <button
            type="button"
      ...
      {/* Mobile-only FAB to open panel when there are invoices */}
      {invoices.length > 0 && !mobilePanelOpen && (
        <button
          type="button"
          onClick={() => setMobilePanelOpen(true)}
          className="md:hidden fixed bottom-24 right-4 z-30 flex items-center gap-2 rounded-full border border-emerald-500/40 bg-emerald-500/15 backdrop-blur-md px-4 py-2.5 text-sm font-medium text-emerald-300 shadow-lg shadow-emerald-500/20 hover:bg-emerald-500/25 transition-colors"
        >
